...
class Bot:
    ...
    def login(self):
        ...
        if (login_json.get("authenticated") and login.status_code == 200):
            ...
            self.user_id = self.get_userid_by_username(self.username)
        else:
            print("Login error")
    ...
    def get_mediaids_by_tag(self, tag):
        ...
        if self.loggedin:
            ...
            for media in media_data:
                mediaids_by_tag.append(media["node"]["id"])
    ...
    def get_userids_by_tag(self, tag):
        ...
        if self.loggedin:
            ...
            media_data = json.loads(i.text)["graphql"]["hashtag"]["edge_hashtag_to_media"]["edges"]
            for media in media_data:
                userids_by_tag.append(media["node"]["owner"]["id"])

        return userids_by_tag

    def get_followerids(self, user_id, amount=50):
        """
    ...
    def get_followingids(self, userid, amount=50):
        ...
        try:
            ...
            for data in following_data:
                following_ids.append(data["node"]["id"])
        except KeyError:
            print("ERROR: " + json.loads(info.text)["message"])
    ...
    def unfollow_if_no_followback(self):
        ...
        user_ids = self.get_follower_ids(self.user_id)
        for user_id in user_ids:
            if not self.is_followed_by(user_id):
